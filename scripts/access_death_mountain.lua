...
function adv_glitched_access_dm_west_top()
    ...
    -- Vanilla
    if has("bluemail") or has_bottle() then return true end
...
function hell_access_dm_west_top()
    ...
    if weather_vane_lorule("wv_treacherous_tower") and access_lorule_not_cracksanity() and hasAll({ "hookshot", "bombs" }) then return true end
    return false
end
...
function access_dm_east_top()
    ...
    if access_lorule_not_cracksanity() and has("hookshot") then
...
function adv_glitched_access_dm_east_top()
    ...
    if (has("bluemail") or has_bottle()) and has("hookshot") then return true end
...
function adv_glitched_access_dm_east_bottom()
    ...
    if (has("bluemail") or has_bottle()) and has("hookshot") and (has("merge") or (hasAll({ "trod", "boots" }) or has("nicebombs"))) then return true end
...
function glitched_access_ldm_east_base()
    ...
    if access_lorule_not_cracksanity() and has("hookshot") then return true end
...
function access_ldm_east_top()
    ...
    if access_lorule_not_cracksanity() and has("hookshot") then
        -- Vanilla
        if has("power_glove") or weather_vane("wv_death_hyrule") or weather_vane("wv_hera") then return true end
    end
